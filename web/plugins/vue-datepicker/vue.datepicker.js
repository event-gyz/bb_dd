...
(function(factory) {
    ...
    Vue.component('mz-datepicker', {
        template: '<div class="mz-datepicker" :style="{width:width+\'px\'}">' +
            '<input :value="value" :name="name" :field="name"  readonly :disabled="disabled" :class="{focus:show}" @click="click" @mousedown="$event.preventDefault()"/>'+
            '<a v-if="clearable&&value" @click.stop="clear"></a><i @click="click"></i>' +
        ...
        methods: {
            ...
            initRanges: function() {
                ...
                ranges.push({
                    name: '最近7天',
                    start: this.parse(new Date(), false),
                    end: this.parse(time, true)
                });
                // time = new Date();
                ...
                ranges.push({
                    name: '下个月',
                    start: new Date(time.getFullYear(), time.getMonth()+2, 1),
                    end:  new Date(time.getFullYear(), time.getMonth()+2, time.getDate())
                });
                // time = new Date();
