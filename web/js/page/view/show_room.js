define(function(require, exports) {
   ...
   var shopCartManage = require('/js/moudle/shop_cart_manage.js')
   ...
   shopCartManage.init('#shopCartBox')
    ...
    var ShopCartControl = {
        addEvt: function(e) {
            console.log(shopCartManage.data.length)
    ...
    function bingEvt() {
        ...
        $(document).on('click', '.add_to_cart_btn', ShopCartControl.addEvt)
        ...
        var start = {
                ...
                istoday: false,
                isclear: false, 
                start: '2016-09-16 23:59:59',    //开始日期
                choose: function(datas) {
                    end.min = datas; //开始日选好后，重置结束日的最小日期
                    end.start = datas //将结束日的初始值设定为开始日
                roomData.checkin=datas
                }
            };
        var end = {
            elem: '#end',
            ...
            choose: function(datas) {
              ...
              roomData.checkout=datas

            }
        };
        roomData.checkin=laydate.now(+1);
        roomData.checkout=laydate.now(+2);

        $('#start').val(laydate.now(+1)).on('blur',function(){
            if($(this).val()==''){
                $(this).val(laydate.now(+1))
            }
        })
        $('#end').val(laydate.now(+2)).on('blur',function(){
            if($(this).val()==''){
                $(this).val(laydate.now(+1))
            }
        })
